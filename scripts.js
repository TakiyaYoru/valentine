...
(function () {
  ...
  if (e) {
    try {
      // Fix: URLSearchParams converts + to space per spec, restore for base64
      // Also no need for decodeURIComponent since URLSearchParams already decodes
      const base64Clean = e.replace(/ /g, '+');
      const decoded = decodeURIComponent(escape(atob(base64Clean)));
      ...
      console["error"]("Dữ liệu không hợp lệ:", err);
      console["warn"]("Sử dụng dữ liệu mặc định thay thế.");
      // Don't set to null, keep the default data
    }
  } else {
    console["warn"](
...
// Music player initialization (works for both URL parameter and default data)
if (window["Heartlove"] && window["Heartlove"]["data"]["music"]) {
  const audio = new Audio(window["Heartlove"]["data"]["music"]);
  audio.loop = true;
...
// Ensure window.Heartlove is properly initialized before accessing
if (!window["Heartlove"] || !window["Heartlove"]["data"]) {
  window.Heartlove = {
    data: {
      messages: ["Em là cả thế giới của anh 💕", "Mãi bên em 🌹", "Anh yêu em ❤️"],
      images: ["https://cdn.shopify.com/s/files/1/0757/9700/4572/files/IMG_5022.jpg?v=1758716338"],
      heartColor: "#ff9090",
      music: "https://cdn.shopify.com/s/files/1/0757/9700/4572/files/b4f4cdf2-91cf-42a8-a422-46b454f25c95.mp3?v=1748781067"
    },
  };
}
...
const p1 = new THREE["PointLight"](16777215, 1.8);
p1["position"]["set"](10, 10, 10);
scene["add"](p1);
const p2 = new THREE["PointLight"](0xff69b4, 1.2);
...
function mainAnimationLoop() {
  ...
  // Animate twinkling stars with enhanced brightness variation
  starLayers.forEach((starLayer) => {
    if (starLayer.visible && starLayer.geometry) {
      const alphaAttr = starLayer.geometry.getAttribute('alpha');
      const phases = starLayer.geometry.userData.phase;
      const twinkleSpeeds = starLayer.geometry.userData.twinkleSpeed;

      if (alphaAttr && phases && twinkleSpeeds) {
        for (let i = 0; i < alphaAttr.count; i++) {
          // Increase speed multiplier from 2 to 4 for faster twinkling
          phases[i] += twinkleSpeeds[i] * e * 4;

          // Enhanced twinkle effect: oscillates between 0.1 (dim) and 1.2 (bright)
          const twinkle = 0.1 + 1.1 * (Math.sin(phases[i]) * 0.5 + 0.5);
          alphaAttr.setX(i, twinkle);
        }
  ...
  // Old shooting stars code removed - using CSS shooting stars instead
  if (heartbeatEnabled) {
...
starPhase = null;
starGeo = null;
// Old Three.js shooting stars removed - using CSS shooting stars instead
...
// Removed image validation code that was causing "Hệ thống đang cập nhật" overlay
fadeObjects["push"](streamHeart);
[streamHeart]["forEach"]((t) => {
  if (t) {
    t.visible = false;
    t["traverse"]?.((t) => {
      if (t["material"] && undefined === t.userData["baseOpacity"]) {
